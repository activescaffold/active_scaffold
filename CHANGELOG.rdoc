- Add support for local sorting using JS when the list has no pagination
- Add support to change tags and HTML attributes of some rendered elements, to use CSS frameworks such as tailwindcss or bootstrap
- Fix draggable lists to set hover class on the list while dragging
- Support defining more options for jQuery.sortable with ActiveScaffold.js_config[:draggable_lists_options]
- Support changing highlight duration with ActiveScaffold.js_config[:highlight]
- ActiveScaffold now calls order with hash, leaving the quoting of tables and columns to rails, Arel.sql must be called on the values set on sort_by if rails requires it
- Column search_sql accepts a symbol and uses symbol by default, delay quoting until query is generated, strings are still supported for cross-table columns or functions
- Use nonce: true in active_scaffold_javascript_tag so rails app can setup CSP with nonce in script-src
- Filtering by params support using range syntax (A..B) supports infinite ranges now, such as ..A or A..
- Speed up listing when all records fit in the first page and it's not full, count query can be avoided.
- Better PostgreSQL support, accept condition from URL for timestamp and timestamptz columns from PostgreSQL, as done with datetime MySQL columns
- Applied and ignored filters (due to be disallowed or unknown) are tracked with @filter_states
- Fix cache column counts on PostgreSQL when the association has order
- Fix list with ordering using sql functions and left joins in PostgreSQL
- Columns with includes and searching in other table won't have includes eager loaded in the same query when they are in the list columns, as may load only the records matching the search. Also, makes aliases when searching in multiple associations on the same table not depending on what columns are in the list columns.
- Support configuring the text search operator per column, defaulting to LIKE for collated PostgreSQL columns
...
= 4.0.10
- Fix override_columns without block
