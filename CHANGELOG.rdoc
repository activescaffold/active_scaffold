...
= 3.6.18 (not released yet)
- Use always column.search_joins instead of column.includes on search actions, search_joins default to includes, so won't change behaviour on most cases, but allows to preload some associations without joining in the query
