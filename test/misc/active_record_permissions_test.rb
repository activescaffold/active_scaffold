...
class ActiveRecordPermissionsTest < Test::Unit::TestCase
  ...
  def test_method_combinations_with_default_true
    ...
    pass(@model.authorized_for?(:action => :create, :column => :a2), 'aft')
    ...
    pass(@model.authorized_for?(:action => :read, :column => :a2), 'tft')
