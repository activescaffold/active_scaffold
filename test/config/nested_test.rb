...
class Config::NestedTest < Test::Unit::TestCase
  ...
  def test_add_link
    @config.nested.add_link :custom_link, [:assoc_1, :assoc_2]
    ...
    assert !link.confirm?
    assert link.inline?
    assert_equal 'assoc_1 assoc_2', link.parameters[:associations]
    assert_equal 'assoc_1 assoc_2', link.html_options[:class]
