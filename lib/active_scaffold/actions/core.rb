module ActiveScaffold::Actions
  module Core
    ...
    def new_model
      model = beginning_of_chain.klass
      config = active_scaffold_config_for(model) if nested? && nested.plural_association?
      if config && config._columns_hash[column = model.inheritance_column]
